/*
  # Fix audit trigger with detailed error logging
  
  1. Changes
    - Add RAISE NOTICE statements to debug what's happening
    - Make sure trigger doesn't silently fail
    - Log the actual error that's occurring
  
  2. Testing
    - Trigger will now show detailed notices when it runs
*/
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE WARNING 'Could not get user email: %', SQLERRM;
  END;
  ...
  -- Verify we have an organization_id
  IF v_organization_id IS NULL THEN
    RAISE WARNING 'No organization_id found for % operation on table %', TG_OP, TG_TABLE_NAME;
    IF TG_OP = 'DELETE' THEN
      RETURN OLD;
    ELSE
      RETURN NEW;
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    LOOP
      ...
      IF v_key NOT IN ('updated_at', 'last_modified', 'modified_by', 'created_at', 'id', 'organization_id') THEN
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE WARNING 'Failed to insert audit log for % on %: % (org_id: %, user_id: %)', 
      TG_OP, TG_TABLE_NAME, SQLERRM, v_organization_id, auth.uid();
  END;
